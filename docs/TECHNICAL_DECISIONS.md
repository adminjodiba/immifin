...
- Green Card Wait Time on-page SEO lives in server-rendered `components/GreenCardWaitTimeSeoContent.tsx` below the calculator. Search metadata stays the existing current-status title/description. H1 is `Green Card Wait Time Calculator`. Free/manual means the user enters category, country, and priority date; IMMIFIN supplies the current published Final Action Date for every user. Pro/Power may autofill those inputs. Do not describe current FAD as Pro-exclusive, add a cutoff-entry field, hardcode current cutoff dates, add FAQPage JSON-LD, or create a second SEO data path.
